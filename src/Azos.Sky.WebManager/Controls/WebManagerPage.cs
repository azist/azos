...
namespace Azos.Sky.WebManager.Controls
{
  ...
  public abstract class WebManagerPage : WebManagerTemplate
  {
    ...
    {
      if (this is TPage)
       Context.Response.WriteAsync("href='#' class='selectedPage'").RunSynchronously();
      else
      {
       Context.Response.WriteAsync("href='").RunSynchronously();
       Context.Response.WriteAsync(uri).RunSynchronously();
       Context.Response.WriteAsync("'").RunSynchronously();
      }
    }
    ...
    public string FormJSON(Form form, Exception validationError = null, string recID = null, string target = null)
    {
      ...
      return RecordModelGenerator.DefaultInstance.RowToRecordInitJSON(form, validationError, Atom.Encode(lang), recID, target).ToJson();
    }
