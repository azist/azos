...
namespace Azos.Log.Sinks
{
  ...
  /// </summary>
  public class FactArchiveSink : VfsSink
  {
    protected FactArchiveSink(ISinkOwner owner) : base(owner) { }
    protected FactArchiveSink(ISinkOwner owner, string name, int order) : base(owner, name, order) { }

    protected IVolume m_Volume;
    protected FactArchiveAppender m_Appender;
    ...
    protected virtual VolumeMetadataBuilder DoBuildMetadata()
      => VolumeMetadataBuilder.Make(Name, FactArchiveAppender.CONTENT_TYPE_FACTS)
    ...
    protected override void DoOpenStream()
    {
      ...
      m_Appender = new FactArchiveAppender(m_Volume, App.TimeSource, App.AppId, Platform.Computer.HostName);
    }
    ...
    {
      //Log.Message -> Log.Fact conversion is amortized by the archive-writing party
      var fact = ArchiveConventions.LogMessageToFact(msg);
      m_Appender.Append(fact);
    }
  }
}
