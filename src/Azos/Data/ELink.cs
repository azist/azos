...
";
    ...
    public ELink(ulong id, byte[] metadata)
    {
      ...
      if (m_Metadata != null && ((m_Metadata.Length * 2) > MAX_LINK_CHAR_SIZE))
        throw new DataException(StringConsts.ELINK_CHAR_LENGTH_LIMIT_ERROR.Args("metadata[{0}]".Args(m_Metadata.Length)));
    }
    ...
    {
      if (link.IsNullOrWhiteSpace()) throw new DataException(StringConsts.ARGUMENT_ERROR + "ELink.ctor(link=null|empty)");

      if (link.Length > MAX_LINK_CHAR_SIZE)
        throw new DataException(StringConsts.ELINK_CHAR_LENGTH_LIMIT_ERROR.Args(link.Substring(0, 20)));
    ...
    /// </summary>
    public ulong ID { get { return m_GDID.ID; } }
    ...
    public string Encode(byte? seed = null) //props -> link
    {
      ...
      int rnd = (seed ?? (byte)(Platform.RandomGenerator.Instance.NextRandomInteger & 0xff)) & VARIATIONS;
      var era = m_GDID.Era;
      var id = m_GDID.ID;
