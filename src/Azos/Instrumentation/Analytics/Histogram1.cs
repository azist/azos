...
namespace Azos.Instrumentation.Analytics
{
  ...
  public class Histogram<TData1> : Histogram
  {
    ...
    /// </summary>
    public override int DimensionCount => 1;

    /// <summary>
    /// Return the sample count associated with given histogram keys
    /// </summary>
    public new int this[int key] => this[new HistogramKeys(key)];
    ...
    /// </summary>
    public bool TryGet(int key, out int count) => TryGet(new HistogramKeys(key), out count);

    /// <summary>
    /// Increment histogram statistics for a given dimension value
    /// </summary>
    public virtual void Sample(TData1 value) => DoSample(Keys(value));

    /// <summary>
    /// Convert a value to HistogramKeys struct
    /// </summary>
    public HistogramKeys Keys(TData1 value) => new HistogramKeys(m_Dimension1[value]);
