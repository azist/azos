...
namespace Azos.CodeAnalysis.JSON
{
  ...
  public class JsonToken : Token
  {
    ...
    public override bool IsPrimary => !IsNonLanguage && Type != JsonTokenType.tComment;

    public override bool IsNonLanguage => Type > JsonTokenType.NONLANG_START && Type < JsonTokenType.NONLANG_END;

    public override int OrdinalType => (int)Type;

    public override bool IsTextualLiteral => Type == JsonTokenType.tStringLiteral;

    public override bool IsNumericLiteral => Type > JsonTokenType.NUMLITERALS_START && Type < JsonTokenType.NUMLITERALS_END;
