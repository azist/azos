...
namespace Azos.CodeAnalysis.CSharp
{
  ...
  public class CSToken : Token
  {
    ...
    public override Language Language => CSLanguage.Instance;

    public override string ToString() => "{0} {1}".Args(Type, base.ToString());
    ...
    public override bool IsPrimary => !IsNonLanguage && Type != CSTokenType.tComment;

    public override bool IsNonLanguage => Type > CSTokenType.NONLANG_START && Type < CSTokenType.NONLANG_END;

    public override int OrdinalType => (int)Type;

    public override bool IsTextualLiteral => Type == CSTokenType.tStringLiteral;

    public override bool IsNumericLiteral => Type > CSTokenType.NUMLITERALS_START && Type < CSTokenType.NUMLITERALS_END;
