...
namespace Azos.IO.FileSystem.Packaging
{
  ...
  public class LocalInstallation : DisposableObject
  {
      ...
      public LocalInstallation(IApplication app, string rootPath, string localManifestDir = null)
      {
        ...
        if (!parent.Exists)
          throw new AzosIOException(StringConsts.LOCAL_INSTALL_ROOT_PATH_NOT_FOUND_ERROR.Args(parent.FullName));
        ...
        if (Directory.Exists(manifestDir))
        {
          ...
          {
            throw new AzosIOException(StringConsts.LOCAL_INSTALL_LOCAL_MANIFEST_READ_ERROR.Args(fn, error.ToMessageWithType()), error);
          }
        }
      ...
      public void EndInstallation()
      {
        ...
        if (File.Exists(fn))
           throw new AzosIOException(StringConsts.LOCAL_INSTALL_PACKAGES_MANIFEST_FILE_NAME_COLLISION_ERROR.Args(fn));
