...
namespace Azos.IO.ErrorHandling
{
  public class GaloisPolynomial
  {
    #region .ctor
      ...
      {
        if (field == null)
          throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".ctor(field=null)");

        if (coefficients == null || coefficients.Length == 0)
          throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".ctor(coeff=null|empty)");
    ...
    #region Public
      ...
      public GaloisPolynomial Add(GaloisPolynomial summand)
      {
        if (!m_Field.Equals(summand.m_Field))
          throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".Add(summand: different field)");
      ...
      public GaloisPolynomial Multiply(GaloisPolynomial factorPolynomial)
      {
        if (!m_Field.Equals(factorPolynomial.m_Field))
          throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".Mult(factor: different field)");
      ...
      public void Divide(GaloisPolynomial divider, out GaloisPolynomial quotient, out GaloisPolynomial remainder)
      {
        if (!m_Field.Equals(divider.m_Field))
          throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".Div(divider: different field)");

        if (divider.IsPolynomial0)
          throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".Div(divider: div zero is undefined)");
