...
namespace Azos.Templatization
{
  public class DOMGenerator : IConfigurable
  {
    ...
    private string createElement(IConfigSectionNode node, bool isRootNode, string container, ref int counter, int extraSpaces, out string elId)
    {
      ...
      else
      {
        sb.AppendFormat("{0}var {1}{4}={4}{5}.ce('{2}');{3}",
                        getIndent(extraSpaces),
                        elemId,
                        WebUtils.EscapeJSLiteral(nName),
                        LineEnding,
                        Space,
                        WaveJS);
        ...
        foreach(var attr in node.Attributes)
        {
          ...
          var name = WebUtils.EscapeJSLiteral(attr.Name);
    ...
    {
      return value.StartsWith(CONFIG_JS_PREFIX_ATTR) ? jsWrap(value, currentElemId) : "'{0}'".Args(WebUtils.EscapeJSLiteral(value));
    }
