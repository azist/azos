...
namespace Azos.Serialization.JSON
{
  ...
  public class JsonWritingOptions : IConfigurable
  {
    ...
    public JsonWritingOptions(JsonWritingOptions other)
    {
      ...
      this.EnableTypeHints          = other.EnableTypeHints;
    }
    ...
    /// <summary>
    /// When set to true, adds type hint annotations into the written json data
    /// decorating the primitive values such as Atoms, Dates, GDIDs, etc..
    /// this way data may round-trip via Json maps/arrays preserving its type identity.
    /// For example a value "$dt:01/01/1980" is going to be written into json stream, so it can be read back as `DateTime` and not as `string`.
    /// Default is `false`, which is the standard Json behavior (turn off type hints)
    /// </summary>
    [Config]
