...
{
  /// <summary>
  /// Represents Mongo Db collections which allows to execute queries and perform CRUD operations
  /// </summary>
  ...
  public sealed class Collection : DisposableObject, INamed
  {
    ...
    internal Collection(Database database, string name)
    {
      ...
      //precompute this in .ctor not to do this on every send
      m_FullNameCStringBuffer = BinUtils.WriteCStringToBuffer(m_FullName);
    }

    protected override void Destructor()
    {
      if (!m_Database.Disposed)
    ...
    #region Props
    public ServerNode  Server   => m_Database.Server;
    ...
    /// </summary>
    /// <param name="query">Query to match against</param>
    /// <param name="skipCount">How many document to skip at the beginning</param>
