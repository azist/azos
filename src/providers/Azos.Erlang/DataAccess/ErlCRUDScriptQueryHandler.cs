  ...
  public sealed class ErlCRUDScriptQueryHandler : CRUDQueryHandler<ErlDataStore>
  {
    ...
    #region ICRUDQueryHandler
      ...
      public override RowsetBase Execute(ICRUDQueryExecutionContext context, Query query, bool oneRow = false)
      {
        ...
        //]};
        return ErlStore.Map.ErlCRUDResponseToRowset(schema, rows, query.ResultDocType);
      }
    ...
    #region .pvt
      ...
      private void checkForError(IErlObject pattern, IErlObject response, ErlVarBind bind, int reqID)
      {
        ...
        switch (ecode)
        {
          case INVALID_SUBSCRIPTION_REQUEST_EXCEPTION:
            error = new Subscriptions.InvalidSubscriptionRequestException(emsg, null);
            break;
          case -1:
      ...
      private static parsedQuery prepareQuery(QuerySource qSource)
      {
         ...
         try
         {
           var mfa = Azos.Erlang.ErlObject.ParseMFA(src);
