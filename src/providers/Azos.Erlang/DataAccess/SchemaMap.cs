...
namespace Azos.Data.Access.Erlang
{
  ...
  public class SchemaMap
  {
    ...
    /// <summary>
    /// Converts ErlCRUD response to CLR rowset
    /// </summary>
    /// <remarks>
    ...
    /// ]
    /// </remarks>
    public RowsetBase ErlCRUDResponseToRowset(string schemaName, ErlList erlData, Type tDoc = null)
    {
      var crudSchema = GetCRUDSchemaForName(schemaName);
      var tSchema    = tDoc == null
        ? crudSchema
        : Schema.GetForTypedDoc(tDoc);
      ...
      foreach(var elm in erlData)
      {
        ...
        if (tDoc != null)
        {
          var trow = Doc.MakeDoc(tSchema, tDoc);
          row.CopyFields(trow);
          row = trow;
        }
        result.Add( row );
    ...
    /// <summary>
    /// Maps ErlRow to CLR data doc supplied by schema, either Dynamic or TypedDoc
    /// </summary>
    public Doc ErlTupleToRow(string schemaName, ErlTuple tuple, Schema schema)
    {
      var singleKey = schema.ExtraData[SCHEMA_KEY_COUNT].AsInt(0) < 2;

      var row = Doc.MakeDoc(schema, schema.TypedDocType);
    ...
    /// </summary>
    public ErlTuple DocToErlTuple(Doc doc, bool keysOnly = false)
    {
      var result = new ErlTuple();

      result.Add(new ErlAtom(doc.Schema.Name));

      var keys = new List<IErlObject>();

      foreach(var def in doc.Schema)
      {
        ...
        var clrValue = doc.GetFieldValue(def);
        if (clrValue!=null)
    ...
    private static DateTime adjustDateToUTC(DateTime dt)
    {
      if (dt.Kind==DateTimeKind.Local)
        dt = Azos.App.LocalizedTimeToUniversalTime(dt);
