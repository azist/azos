...
namespace Azos.Erlang
{
  ...
  #region Public
    ...
    public static ErlPid Parse(string pid)
    {
      ErlPid result;
      if (TryParse(pid, out result)) return result;

      throw new ErlException(StringConsts.ARGUMENT_ERROR + "ErlPid.Parse('{0}')".Args(pid));
    }

    public static bool TryParse(string pid, out ErlPid epid)
    {
      epid = ErlPid.Null;
      if (pid.IsNullOrWhiteSpace() || pid.Length < 5) return false;

      if (pid[0] == '#') pid = pid.Substring(5, pid.Length - 5 - 1);
      ...
      int id;
      if (!Int32.TryParse(segs[1], out id)) return false;

      int serial;
      if (!Int32.TryParse(segs[2], out serial)) return false;

      int creation;
      if (!Int32.TryParse(segs[3], out creation)) return false;


      epid = new ErlPid(node, id, serial, creation);
      return true;
    }
    ...
    {
      return string.Format("#Pid<{0}.{1}.{2}.{3}>", Node, Id, Serial, Creation);
    }
