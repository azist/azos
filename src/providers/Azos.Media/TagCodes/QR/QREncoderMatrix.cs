    ...
    #region Static
                                                         ...
                                                         int blockID, int[] numDataBytesInBlock, int[] numECBytesInBlock)
      {
        if (blockID >= numRSBlocks)
          throw new AzosException(StringConsts.ARGUMENT_ERROR + typeof(QREncoderMatrix).Name + ".getNumDataBytesAndNumECBytesForBlockID(blockID<numRSBlocks)");
        ...
        if (numEcBytesInGroup1 != numEcBytesInGroup2)
          throw new AzosException(StringConsts.CODE_LOGIC_ERROR + typeof(QREncoderMatrix).Name +
