  ...
  public partial class QRMatrix: Matrix2D<byte>
  {
    ...
    #region Public
      ...
      public void InsertDataBits(BitList dataBits, int maskPattern)
      {
        ...
        if (bitIndex != dataBits.Size)
          throw new AzosException(StringConsts.CODE_LOGIC_ERROR + GetType().Name + ".makeVersionInfoBits: bitIndex != dataBits.Size");
      }
    ...
    #region .pvt. impl.
      ...
      private void addHorizontalSeparationPattern( int xStart, int yStart)
      {
        for (int x = 0; x < 8; ++x)
        {
          if (this[xStart + x, yStart] != EMPTY_BYTE)
            throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".addHorizontalSeparationPattern(this[i]!=EMPTY_BYTE)");
      ...
      private void addVerticalSeparationPattern( int xStart, int yStart)
      {
        for (int y = 0; y < 7; ++y)
        {
          if (this[xStart, yStart + y] != EMPTY_BYTE)
            throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".addVerticalSeparationPattern(this[i]!=EMPTY_BYTE)");
      ...
      private void addDarkDotAtLeftBottomCorner()
      {
         if (this[8, Height - 8] == 0)
            throw new AzosException(StringConsts.ARGUMENT_ERROR + GetType().Name + ".addDarkDotAtLeftBottomCorner(this[i]!=0)");
