...
namespace Azos.Wave.Handlers
{
  ...
  public abstract class TypeLookupHandler<TTarget> : WorkHandler where TTarget : class
  {
    ...
    #region .ctor
    protected TypeLookupHandler(WorkHandler director, string name, int order, WorkMatch match = null)
                        : base(director, name, order, match)
    {
    }

    protected TypeLookupHandler(WorkHandler director, IConfigSectionNode confNode) : base(director, confNode)
    {
      if (confNode==null)
    ...
    /// </summary>
    public virtual bool SupportsInstanceID => false;

    /// <summary>
    /// Returns a registry of type locations
    /// </summary>
    public TypeLocations TypeLocations => m_TypeLocations;
    ...
    /// </summary>
    protected sealed override async Task DoHandleWorkAsync(WorkContext work)
    {
      ...
      try
      {
        try
        {
          ...
          await DoTargetWorkAsync(target, work).ConfigureAwait(false);
    ...
    /// </summary>
    protected abstract Task DoTargetWorkAsync(TTarget target, WorkContext work);
